  ...
  return (
    <>
      <div className='fixed top-0 right-0 w-11/12 -z-10 translate-y-[-80%] dark:hidden'>
        <Image src={assets.header_bg_color} className='w-full' alt='header-bg'/>
      </div>
      <nav className={`w-full fixed px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50 ${isScrolled ? "bg-white/50 backdrop-blur-lg shadow-sm dark:bg-darkTheme dark:shadow-white/20" : ""} transition duration-500`}>
        <a href="#top">
        ...
        <div className='flex items-center gap-4'>
            <button onClick={() => setIsDarkMode(prev => !prev)} >
              {/* prev => !prev => true to false and false to true */}
                <Image src={ isDarkMode ? assets.sun_icon : assets.moon_icon} className='w-6' alt=''/>
            </button>
            <a href="#contact" className='hidden lg:flex items-center gap-3 px-10 py-2.5 border border-gray-500 rounded-full ml-4 font-Ovo dark:border-white/50'>Contact <Image src={ isDarkMode ? assets.arrow_icon_dark : assets.arrow_icon} className='w-3' alt='arrow'/></a>
